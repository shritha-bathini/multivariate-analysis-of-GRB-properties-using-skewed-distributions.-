file_path <- 'C:/Users/Dell/My Files/Rfiles/project/swift_data.csv'
...
y = na.omit(y)
length(y)
brks <- seq(-5,10,by=0.25)
hist(log(y),breaks=brks)
library(mixsmsn)
library("sn")
library("stats")
AIC<- numeric()
...
for (n in 2:8)
{
  ...
  Skewnorm.analysis <- smsn.mix(t,nu=NULL,g=n,get.init=TRUE,group=TRUE,calc.im=FALSE,family="Normal")
...
#skew-normal
for (n in 2:5)
{ Skewnorm.analysis <- smsn.mix(t,nu=NULL,g=n,get.init=TRUE,group=TRUE,calc.im=FALSE,family="Skew.normal")
aic <- Skewnorm.analysis$aic
bic <- Skewnorm.analysis$bic
AIC <- append(AIC,aic)
BIC<- append(BIC,bic)
...
#t
for (n in 2:5)
{ Skewnorm.analysis <- smsn.mix(t,nu=6,g=n,get.init=TRUE,group=TRUE,calc.im=FALSE,family="t")
aic <- Skewnorm.analysis$aic
bic <- Skewnorm.analysis$bic
AIC <- append(AIC,aic)
BIC<- append(BIC,bic)
...
#skewed-t
for (n in 2:5)
{ Skewnorm.analysis <- smsn.mix(t,nu=6,g=n,get.init=TRUE,group=TRUE,calc.im=FALSE,family="Skew.t")
aic <- Skewnorm.analysis$aic
bic <- Skewnorm.analysis$bic
AIC <- append(AIC,aic)
BIC<- append(BIC,bic)
